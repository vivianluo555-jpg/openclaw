...
export const video = table(
  'video',
  {
    ...
    userId: varchar191('user_id')
      .notNull()
      .references(() => user.id, { onDelete: 'cascade' }),
    youtubeId: varchar191('youtube_id').unique().notNull(),
    title: varchar('title', { length: 255 }).notNull(),
    description: text('description'),
    thumbnailUrl: text('thumbnail_url'),
    contentCategory: varchar('content_category', { length: 50 }),
    ipCategory: varchar('ip_category', { length: 50 }),
    styleCategory: varchar('style_category', { length: 50 }),
...
export const videoBreakdown = table(
  ...
  {
    id: varchar191('id').primaryKey(),
    videoId: varchar191('video_id')
      .notNull()
      .references(() => video.id, { onDelete: 'cascade' }),
    scriptContent: longtext('script_content'),
    hookElements: longtext('hook_elements'),
    emotionalCurve: longtext('emotional_curve'),
    pacingAnalysis: longtext('pacing_analysis'),
    keyTakeaways: longtext('key_takeaways'),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().onUpdateNow().notNull(),
  },
  (table) => [
