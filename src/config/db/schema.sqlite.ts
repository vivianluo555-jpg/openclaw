...
export const video = table(
  'video',
  {
    ...
    ipCategory: text('ip_category'),
    styleCategory: text('style_category'),
  ...
  (table) => [
    index('idx_video_user_id').on(table.userId),
    index('idx_video_content_category').on(table.contentCategory),
    index('idx_video_ip_category').on(table.ipCategory),
    index('idx_video_style_category').on(table.styleCategory),
    index('idx_video_created_at').on(table.createdAt),
  ]
);

export const videoBreakdown = table(
  'video_breakdown',
  {
    id: text('id').primaryKey(),
    ...
    hookElements: text('hook_elements'),
    emotionalCurve: text('emotional_curve'),
    pacingAnalysis: text('pacing_analysis'),
    keyTakeaways: text('key_takeaways'),
    // Hotvault-specific fields
    hook: text('hook'), // Golden 3-second hook description
    retentionStrategy: text('retention_strategy'), // Retention strategy text
    coreLogic: text('core_logic'), // Core logic analysis
    viralFormula: text('viral_formula'), // Reusable viral formula string
    replicability: integer('replicability'), // 1-5 star rating
    viralElements: text('viral_elements'), // JSON array of element tags
    imagePrompt: text('image_prompt'), // AI image generation prompt
    videoPrompt: text('video_prompt'), // AI video generation prompt
    createdAt: integer('created_at', { mode: 'timestamp_ms' })
      .default(sqliteNowMs)
      .notNull(),
    updatedAt: integer('updated_at', { mode: 'timestamp_ms' })
  ...
  (table) => [
    index('idx_video_breakdown_video_id').on(table.videoId),
  ]
);
